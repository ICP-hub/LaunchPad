...
const FormModal = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
        const actor = await createCustomActor(process.env.CANISTER_ID_ICPLAUNCHPAD_BACKEND);
        ...
        // Use fetchWithRetry for submitting the payload
        const response = await fetchWithRetry(() => actor?.add_data(payload2), 3,1000 );
